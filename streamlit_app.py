...
TYPE_ICONS = {
    75: "fire", 76: "flag", 77: "plane", 78: "camera",
...
def get_hotel_images(api_key, content_id):
    ...
    params = {
        "ServiceKey": api_key, "MobileOS": "ETC", "MobileApp": "hotel_app",
        "contentId": content_id, "imageYN": "Y", "_type": "json"
    }
    try:
        res = requests.get(url, params=params)
        data = res.json()
...
if page == "호텔 정보":
    ...
    **주변 관광지 수:** {hotel_info['tourist_count']}
    ...
    st.markdown(f"""
    <div style=" padding: 15px; border: 2px solid #d3d3d3; background-color: #f0f0f0;
    ...
    with col1:
        ...
        for _, row in tourist_df.iterrows():
            ...
            if highlight:
                icon = BeautifyIcon(
                    icon="star", icon_shape="marker", border_color="yellow",
                    text_color="white", background_color="yellow", prefix="fa", icon_size=[30,30]
                )
            else:
                icon = BeautifyIcon(
                    icon=icon_name, icon_shape="circle", border_color=row["color"],
                    text_color="white", background_color=row["color"], prefix="fa", icon_size=[20,20]
                )
            folium.Marker(
    ...
    with col2:
        legend_html = """
        <div style=" background-color: white; border:2px solid grey; padding: 10px;
                    border-radius: 5px; box-shadow: 3px 3px 6px rgba(0,0,0,0.3); font-size: 16px; ">
            <b>[관광지 범례]</b><br>
        """
    ...
    if not tourist_df.empty:
        ...
        for t_type, group in tourist_df.groupby("type_name"):
            ...
            temp["구글 지도"] = temp.apply(
                lambda x: f'<a href="https://www.google.com/maps/search/{x["name"].replace(" ","+")}" target="_blank">지도 보기</a>',
                axis=1
            )
        ...
        st.write(
            final_df.to_html(index=False, escape=False, justify="center").replace("<th>", "<th style='text-align:center'>"),
            unsafe_allow_html=True
        )
    else:
