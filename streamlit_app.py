...
# hotel_info 가져오기 시 tourist_count 없는 경우 대비
hotel_info = hotels_df.loc[hotels_df["name"]==selected_hotel].copy()
if "tourist_count" not in hotel_info.columns:
    hotel_info["tourist_count"] = 0
...
""", unsafe_allow_html=True)
    ...
    category_list = ["선택 안 함"] + (tourist_df["type_name"].unique().tolist() if not tourist_df.empty else [])
